...
Message *Message::decodeBytesToMessage(EIoBuffer *input) THROWS(EException) {
    switch (input->get()) {
    case 0:
        return SignupMessage::decodeBytes(input);
    case 1:
        DEBUG("Get Umimplemented SignupResultMessage\n");
        return nullptr;
    case 2:
        return LoginMessage::decodeBytes(input);
    case 3:
        return LoginResultMessage::decodeBytes(input);
    case 4:
        return PullMessage::decodeBytes(input);
    case 5:
        return PullResultMessage::decodeBytes(input);
    case 6:
        return TextMessage::decodeBytes(input);
    case 7:
        return QueryUserInformationMessage::decodeBytes(input);
    case 8:
        return QueryUserResultMessage::decodeBytes(input);
    case 9:
        return RequestFriendMessage::decodeBytes(input);
    case 10:
        return SendRequestMessage::decodeBytes(input);
    case 11:
        return ReplyPermissionMessage::decodeBytes(input);
    case 12:
        return SendReplyMessage::decodeBytes(input);
    case 13:
        return DeleteFriendMessage::decodeBytes(input);
    case 14:
        return QueryFriendListMessage::decodeBytes(input);
    case 15:
        return QueryFriendListResultMessage::decodeBytes(input);
    case 16:
        return LogOutMessage::decodeBytes(input);
    default:
...
EIoBuffer *Message::getEncodedMessage() { DEBUG("BaseMessage Cant be encoded\n"); }

void Message::processMessage() { DEBUG("Base Message havnot any function\n"); }

EString Message::toString() { return EString("Base Message"); }
/* --------------------------------------------------------------------------------------------------
 *      0. SignupMessage
 * --------------------------------------------------------------------------------------------------
 */
...
EString LoginMessage::toString() { return EString("LoginMessage"); }
/* --------------------------------------------------------------------------------------------------
...
void LoginResultMessage::processMessage() {
    ...
    DEBUG("after construct extra\n");
    if (!success) {
        switch (retcode) {
        case -1:
            extra = "Wrong user name or password";
            break;
        case -2:
            extra = "Already Login";
            break;
...
EString PullMessage::toString() { return EString("PullMessage"); }
/* --------------------------------------------------------------------------------------------------
 *      5. PullResultMessage
 * --------------------------------------------------------------------------------------------------
 */
...
PullResultMessage *PullResultMessage::decodeBytes(EIoBuffer *input) {
    ...
    vector<EString> nameList(messageCount, EString(""));
    vector<EString> messageList(messageCount, EString(""));
...
PullResultMessage::PullResultMessage(vector<EString> _usernameList, vector<EString> _contentList)
...
// TODO 解析消息类型，或者直接传递给serverhandler
void PullResultMessage::processMessage() {
    for (int i = 0; i < contentList.size(); ++ i) {
        EIoBuffer *buffer = nullptr;
        buffer = EIoBuffer::allocate(contentList[i].length());
        buffer->put(contentList[i].c_str(), contentList[i].length());
        Message *msg = Message::decodeBytesToMessage(buffer);
        if(TextMessage *tmsg = dynamic_cast<TextMessage*>(msg)){
            tmsg->setUserName(usernameList[i]);
        }
...
EString PullResultMessage::toString() { return EString("PullResultMessgae"); }
/* --------------------------------------------------------------------------------------------------
 *      6. TextMessage
 * --------------------------------------------------------------------------------------------------
 */
...
TextMessage::TextMessage(EString _username, EString _content)
    : username(_username), content(_content) {}
...
QueryUserInformationMessage::QueryUserInformationCallBack
    QueryUserInformationMessage::resultCallBack = nullptr;
...
EString QueryUserInformationMessage::toString() { return EString("QueryUserInformationMessage"); }
/* --------------------------------------------------------------------------------------------------
...
QueryUserResultMessage::QueryUserResultMessage(vector<EString> _userList) : userList(_userList) {}
...
EString QueryUserResultMessage::toString() { return EString("QueryUserResultMessage"); }
/* --------------------------------------------------------------------------------------------------
 *      9. RequestFriendMessage
 * --------------------------------------------------------------------------------------------------
 */
unsigned int RequestFriendMessage::type_num = 9;
// RequestFriendMessage::RequestFriendCallBack
// RequestFriendMessage::resultCallBack = nullptr;
...
RequestFriendMessage::RequestFriendMessage(EString _username, EString _note)
    : username(_username), note(_note) {}
...
EString RequestFriendMessage::toString() { return EString("RequestFriendMessage"); }
/* --------------------------------------------------------------------------------------------------
...
SendRequestMessage::SendRequestMessage(EString _username, EString _note)
    : username(_username), note(_note) {}
...
EString SendRequestMessage::toString() { return EString("SendRequestMessage"); }
/* --------------------------------------------------------------------------------------------------
...
EString ReplyPermissionMessage::toString() { return EString("ReplyPermissionMessage"); }
/* --------------------------------------------------------------------------------------------------
 *      12. SendReplyMessage
 * --------------------------------------------------------------------------------------------------
 */
...
void SendReplyMessage::processMessage() {
    if (TextMessage::resultCallBack) {
        TextMessage::resultCallBack("server friend reply", username.c_str(),
                                    success ? "true" : "false");
    } else {
        DEBUG("Get a nullptr TextMessage callback\n");
    }
...
EString SendReplyMessage::toString() { return EString("SendReplyMessage"); }
/* --------------------------------------------------------------------------------------------------
...
EString DeleteFriendMessage::toString() { return EString("DeleteFriendMessage"); }
/* --------------------------------------------------------------------------------------------------
 *      14. QueryFriendListMessage
 * --------------------------------------------------------------------------------------------------
 */
unsigned int QueryFriendListMessage::type_num = 14;
QueryFriendListMessage::QueryFriendListMessageCallBack QueryFriendListMessage::resultCallBack =
    nullptr;
...
EString QueryFriendListMessage::toString() { return EString("QueryFriendListMessage"); }
/* --------------------------------------------------------------------------------------------------
...
QueryFriendListResultMessage::QueryFriendListResultMessage(vector<EString> _friendList)
...
EString QueryFriendListResultMessage::toString() { return EString("QueryFriendListResultMessage"); }
/* --------------------------------------------------------------------------------------------------
 *      16. LogOutMessage
 * --------------------------------------------------------------------------------------------------
 */
...
EString LogOutMessage::toString() { return EString("LogOutMessage"); }
