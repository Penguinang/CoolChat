...
QPushButton* buttonList[5];
//查找好友界面
AddFriend::AddFriend(Server *server,QWidget *parent,MainWindow *mainwin_): QWidget(parent)
{
    this->mainwin=mainwin_;
...
void AddFriend::callback(vector<struct userinfo> &friends_list)
{
    ...
    if(size > 5){
        printf("UserList too long\n");
        return ;
    }
    for(int i=0;i<size;i++)
    {
        buttonList[i]->setText(QString::fromStdString(friends_list[i].username));
...
void AddFriend::BtnListOnClicked()
{
    ...
    ApplicationWindow* aWindow = new ApplicationWindow(m_server, 0, QString::fromStdString(username),this->mainwin);
    aWindow->show();
}
