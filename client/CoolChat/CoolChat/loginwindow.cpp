...
LoginWindow::LoginWindow(QWidget *parent)
    :QWidget(parent)
{
    auto get_text_callback_bind=std::bind(&MainWindow::get_text_callback,mainwin,placeholders::_1,placeholders::_2,placeholders::_3);
    m_server = new Server(ip,port,get_text_callback_bind);
    ...
    // mainwin = new MainWindow(this->m_server, 0);
    // mainwin->hide();
...
{
    registerWindow = new RegisterWindow(0, m_server);
    registerWindow ->show();
}
...
void LoginWindow::callback(bool success,std::string extra)
{
    if(success == true)
    {
        mainwin->show();
        this->close();
        auto callback_bind=std::bind(&MainWindow::callback,mainwin,placeholders::_1);
        m_server->GetFriendList (callback_bind);
    }
