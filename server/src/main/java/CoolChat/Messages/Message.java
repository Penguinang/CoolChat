...
abstract public class Message {
	...
	public static Message getDecodedMessage(IoBuffer input) throws Exception {
		...
		case 0:// SignUpMessage
			...
			return new SignUpMessage(removeLastChar(new String(userName)),
					new String(e_mail).substring(0, e_mailLength - 1),
					new String(password).substring(0, passwordLength - 1));
		...
		case 2:// loginMessage
			...
			return new LoginMessage(removeLastChar(new String(loginUserName)),
					removeLastChar(new String(loginPassword)));
		...
		case 9:// requestFriendMessage
			...
			return new RequestFriendMessage(removeLastChar(new String(targetUserName)),
					removeLastChar(new String(remark)));
		...
		case 10:// sendRequestMessage
			...
			return new SendRequestMessage(removeLastChar(new String(sourceUserName)),
					removeLastChar(new String(SendRequestRemark)));
		...
		case 11:// ReplyPermissionMessage
			...
			// return new Re(rIsAgree,removeLastChar(new
			// String(ReplyPermissionTargetUserName)),removeLastChar(new
			// String(ReplyPermissionyRemark)));
			return new ReplyPermissionMessage(rIsAgree, removeLastChar(new String(ReplyPermissionTargetUserName)),
					removeLastChar(new String(ReplyPermissionyRemark)));
		...
		case 12:// SendReplyMessage
			...
			return new SendReplyMessage(sIsAgree, new String(SendReplySourceUserName),
					removeLastChar(new String(SendReplyRemark)));
	 ...
	 * @return 消息类型类
	 */
	public static Class<?> peekMessageType(IoBuffer input) {
		int msgType = input.get();
		Class<?> msgTypes[] = { SignUpMessage.class,
				//
				SignUpResultMessage.class,
				//
				LoginMessage.class,
				//
				LoginResultMessage.class,
				//
				PullMessage.class,
				//
				PullResultMessage.class,
				//
				TextMessage.class,
				//
				QueryUserInformationMessage.class,
				//
				QueryUserResultMessage.class,
				//
				RequestFriendMessage.class,
				//
				SendRequestMessage.class,
				//
				ReplyPermissionMessage.class,
				//
				SendReplyMessage.class,
				//
				DeleteFriendMessage.class,
				//
				QueryFriendListMessage.class,
				//
				QueryFriendListResultMessage.class,
				//
				LogOutMessage.class,
				//
		};
		return msgTypes[msgType];
	}
